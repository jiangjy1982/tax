...
if __name__ == '__main__':
    ...
    logging.info(f"    Capital loss carryover: -{rtc.capital_loss_carryover}")
    ...
    other_income = (rtc.taxable_state_refund + rtc.rental_income_offset + rtc.k1_income + rtc.misc_income)
    logging.info(f"Other income: {other_income:.0f}")
    logging.info(f"    Taxable state refunds: {rtc.taxable_state_refund:.0f}")
    logging.info(f"    Rental and K1 income: {(rtc.rental_income_offset + rtc.k1_income):.0f}")
    ...
    logging.info(f"Penalty: {rtc.penalty:.0f}")
